...
#[proc_macro_derive(ImplGetCodeOccurenceFromTufaCommon)]
pub fn derive_impl_get_code_occurence_from_tufa_common(
    input: proc_macro::TokenStream,
) -> proc_macro::TokenStream {
    generate(input, proc_macro_helpers::path::Path::TufaCommon)
}

#[proc_macro_derive(ImplGetCodeOccurenceFromCrate)]
pub fn derive_impl_get_code_occurence_from_crate(
    input: proc_macro::TokenStream,
) -> proc_macro::TokenStream {
...
fn generate(
    ...
    let ast: syn::DeriveInput =
        syn::parse(input).expect("ImplGetCodeOccurence syn::parse(input) failed");
    let ident = &ast.ident;
    let get_code_occurence_trait_token_stream = format!("{path}::traits::get_code_occurence::GetCodeOccurence")
        .parse::<proc_macro2::TokenStream>()
        .expect("path parse failed");
    let code_occurence_token_stream = format!("{path}::common::code_occurence::CodeOccurence")
        .parse::<proc_macro2::TokenStream>()
        .expect("path parse failed");
    match ast.data {
        syn::Data::Union(_) => {
            panic!("ImplGetCodeOccurence does not work on unions!")
        }
        syn::Data::Enum(data_enum) => {
            let variants = data_enum.variants.into_iter().map(|v| {
                ...
                let is_wrapper = if ident_as_string.contains(WRAPPER_NAME) && ident_as_string.contains(ORIGIN_NAME) {
                    panic!("ImplGetCodeOccurence - ident name {} contains {} and {}", ident_as_string, WRAPPER_NAME, ORIGIN_NAME);
                }
                    ...
                    panic!("ImplGetCodeOccurence - ident name {} does not contain {} or {}", ident_as_string, WRAPPER_NAME, ORIGIN_NAME);
                };
                match v.fields {
                    syn::Fields::Unit => panic!(
                        "ImplGetCodeOccurence still not work with syn::Fields::Unit"
                    ),
                        ...
                        let fields_logic = fields_named.named.iter().map(|field_named| {
                            let field_ident = &field_named.ident;
                                match &field_named.ty {
                                    ...
                                    _ => panic!("ImplGetCodeOccurence only work on enums with Path(type_path)!")
                                }
                        });
                    ...
                    syn::Fields::Unnamed(unnamed) => {
                        match unnamed.unnamed.len() {
                            ...
                            _ => panic!("ImplGetCodeOccurence only work on enums with unnamed.len() == 1!")
                        }
                    }
                }
            });
                    ...
                    match fields_named.named.len() {
                        2 => {
                            let source_field_ident = fields_named.named[0]
                                .ident
                                .clone()
                                .expect("ImplGetCodeOccurence - there is no first field ident!");
                            if format!("{}", source_field_ident) != *"source" {
                                panic!("ImplGetCodeOccurence - no 'source'-named field found!");
                            }
                            match fields_named.named[0].ty.clone() {
                                syn::Type::Path(type_path) => {
                                    ...
                                    let generated = quote::quote! {
                                        ...
                                        impl #get_code_occurence_trait_token_stream for #ident {
                                            fn get_code_occurence(&self) -> &#code_occurence_token_stream {
                                                &self.code_occurence
                                            }
                                        }
                                    };
                                ...
                                _ => panic!("ImplGetCodeOccurence only work on Type::Path!"),
                            }
                        }
                        _ => panic!(
                            "ImplGetCodeOccurence only work on structs with 2 named fields!"
                        ),
                    }
                }
                _ => panic!("ImplGetCodeOccurence only work with syn::Fields::Named!"),
            }
        }
    }
}
